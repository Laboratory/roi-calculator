...
const MonthlyROIBreakdown = ({ data }) => {
  ...
  const [selectedScenarios, setSelectedScenarios] = useState(
    data && data.priceScenarios ? data.priceScenarios.map(s => s.name) : []
  );
        ...
        <>
                  ...
                  <div className="scenario-toggles">
                    <Form>
                      <div className="d-flex flex-wrap gap-3">
                        {data.priceScenarios.map(scenario => (
                          <Form.Check
                            key={scenario.name}
                            type="checkbox"
                            id={`scenario-${scenario.name}`}
                            label={scenario.name === 'Bear' ? t('calculator:results.scenarios.bearish') :
                                   scenario.name === 'Base' ? t('calculator:results.scenarios.neutral') :
                                   scenario.name === 'Bull' ? t('calculator:results.scenarios.bullish') : scenario.name}
          ...
          <div className="table-section">
            <OverlayTrigger
              ...
              overlay={
                <Tooltip id="tooltip-monthly-breakdown">
                  {data.unlockFrequency === 'weekly'
                    ? t('calculator:results.monthlyBreakdown.breakdownTooltip.weekly')
                    : t('calculator:results.monthlyBreakdown.breakdownTooltip.monthly')}
                </Tooltip>
              }
            >
                      ...
                      <OverlayTrigger
                        ...
                        overlay={
                          <Tooltip id="tooltip-month-column">
                            {data.unlockFrequency === 'weekly'
                              ? t('calculator:results.monthlyBreakdown.tableColumns.period.weekTooltip')
                              : t('calculator:results.monthlyBreakdown.tableColumns.period.tooltip')}
                          </Tooltip>
                        }
                      >
                      ...
                      <OverlayTrigger
                        ...
                        overlay={
                          <Tooltip id="tooltip-tokens-unlocked">
                            {data.unlockFrequency === 'weekly'
                              ? t('calculator:results.monthlyBreakdown.tableColumns.tokensUnlocked.weekTooltip')
                              : t('calculator:results.monthlyBreakdown.tableColumns.tokensUnlocked.tooltip')}
                          </Tooltip>
                        }
                      >
                        <span className="tooltip-label">
                          ...
                          <OverlayTrigger
                            ...
                            overlay={
                              <Tooltip id={`tooltip-monthly-revenue-${scenario}`}>
                                {data.unlockFrequency === 'weekly'
                                  ? t('calculator:results.monthlyBreakdown.tableColumns.revenue.weekTooltip')
                                  : t('calculator:results.monthlyBreakdown.tableColumns.revenue.tooltip')}
                              </Tooltip>
                            }
                          >
                            <span className="tooltip-label">
                              {scenario === 'Bear' ? t('calculator:results.monthlyBreakdown.scenarioLabels.bear') :
                               scenario === 'Base' ? t('calculator:results.monthlyBreakdown.scenarioLabels.base') :
                               scenario === 'Bull' ? t('calculator:results.monthlyBreakdown.scenarioLabels.bull') : scenario} {t('calculator:results.monthlyBreakdown.tableColumns.revenue.title')}
                              <FaInfoCircle className="ms-2 text-primary info-icon" />
                            </span>
                          </OverlayTrigger>
                        </th>
                          ...
                          <OverlayTrigger
                            ...
                            overlay={
                              <Tooltip id={`tooltip-cumulative-roi-${scenario}`}>
                                {data.unlockFrequency === 'weekly'
                                  ? t('calculator:results.monthlyBreakdown.tableColumns.cumulativeRoi.weekTooltip')
                                  : t('calculator:results.monthlyBreakdown.tableColumns.cumulativeRoi.tooltip')}
                              </Tooltip>
                            }
                          >
                            <span className="tooltip-label">
                              {scenario === 'Bear' ? t('calculator:results.monthlyBreakdown.scenarioLabels.bear') :
                               scenario === 'Base' ? t('calculator:results.monthlyBreakdown.scenarioLabels.base') :
                               scenario === 'Bull' ? t('calculator:results.monthlyBreakdown.scenarioLabels.bull') : scenario} {t('calculator:results.monthlyBreakdown.tableColumns.cumulativeRoi.title')}
                              <FaInfoCircle className="ms-2 text-primary info-icon" />
                            </span>
                          </OverlayTrigger>
                        </th>
                  ...
                  {months.map(month => (
                    <tr key={month} className={Object.entries(breakEvenMonths).some(([scenario, breakMonth]) =>
                      ...
                      <td className={darkMode ? "text-white" : ""}>
                        {data.unlockFrequency === 'weekly'
                          ? (formatDate(month)
                              ? t('calculator:results.monthlyBreakdown.tableColumns.period.weekTitle', { number: getWeekNumber(month), date: formatDate(month) })
                              : t('calculator:results.monthlyBreakdown.tableColumns.period.weekTitle', { number: getWeekNumber(month) }))
                          : formatTimeUnit(month)}
                        {data.unlockFrequency !== 'weekly' && formatDate(month) &&
                          <div className="calendar-date">{formatDate(month)}</div>}
                      </td>
                      <td className={darkMode ? "text-white" : ""}>{data.monthlyUnlocks[month].toLocaleString(undefined, { maximumFractionDigits: 2 })} {data.tokenName}</td>
                      {selectedScenarios.map(scenario => (
                        <React.Fragment key={scenario}>
                          <td className={darkMode ? "text-white" : ""}>{formatCurrency(data.monthlyRevenue[scenario][month])}</td>
                          <td className={data.cumulativeROI[scenario][month] >= 0 ? 'positive-roi' : 'negative-roi'}>
                            ...
                            {breakEvenMonths[scenario] === month && (
                              <span className="break-even-badge">{t('calculator:results.monthlyBreakdown.breakEven')}</span>
                            )}
                          </td>
                        </React.Fragment>
                      ))}
            ...
            <div className="total-summary mt-4">
              <h4 className={`subsection-title ${darkMode ? "text-white" : ""}`}>{t('calculator:results.monthlyBreakdown.totalSummary.title')}</h4>
                    ...
                    <Col md={4} key={scenario} className="mb-3 d-flex">
                      <Card className={`summary-card ${scenario.toLowerCase()}-summary flex-fill`} style={getScenarioStyle(scenario)}>
                        <Card.Body className="d-flex flex-column">
                          <div className={`summary-title ${darkMode ? "text-white" : ""}`}>{scenario === 'Bear' ? t('calculator:results.monthlyBreakdown.scenarioLabels.bear') :
                           scenario === 'Base' ? t('calculator:results.monthlyBreakdown.scenarioLabels.base') :
                           scenario === 'Bull' ? t('calculator:results.monthlyBreakdown.scenarioLabels.bull') : scenario}</div>
                          <div className="summary-content flex-grow-1">
                            <div className="summary-item">
                              <span className={darkMode ? "text-white" : ""}>{t('calculator:results.monthlyBreakdown.totalSummary.totalRevenue')}:</span>
                              <span className={darkMode ? "text-white" : ""}>{formatCurrency(totalRevenue)}</span>
                            </div>
                            <div className="summary-item">
                              <span className={darkMode ? "text-white" : ""}>{t('calculator:results.monthlyBreakdown.totalSummary.finalRoi')}:</span>
          ...
          <div className="key-metrics">
            <div>
              <span className="metric-label">{t('calculator:results.monthlyBreakdown.keyMetrics.initialInvestment')}:</span>
              <span className="metric-value">{data && data.initialInvestment ? formatCurrency(data.initialInvestment) : formatCurrency(0)}</span>
            </div>
            <div>
              <span className="metric-label">{t('calculator:results.monthlyBreakdown.keyMetrics.tokenAllocation')}:</span>
